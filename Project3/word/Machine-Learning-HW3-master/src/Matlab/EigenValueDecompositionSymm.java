...
public class EigenValueDecompositionSymm {
    ...
    public EigenValueDecompositionSymm(
        ...
        for (int i = 0; i < m; i++) {
        	System.out.println("symm1");
            eigenValues[i] = eigenVectors[m - 1][i];
        }
        for (int i = m - 1; i > 0; i--) {
        	System.out.println("symm2");
            double sc = 0.0;
            double h1 = 0.0;
            for (int j = 0; j < i; j++) {
            ...
            if (sc == 0.0) {
                ...
                for (int j = 0; j < i; j++) {
                	System.out.println("symm5 "+j+" "+i);
                    f = eigenValues[j];
                    eigenVectors[j][i] = f;
                    g = accu[j] + eigenVectors[j][j] * f;
                    for (int k = j + 1; k <= i - 1; k++) {
                    	System.out.println("symm6 "+j+" "+k+" "+i);
                ...
                f = 0.0;
                for (int j = 0; j < i; j++) {
                	System.out.println("symm7 "+j+" "+i);
                    accu[j] /= h1;
                    f += accu[j] * eigenValues[j];
                }
                double h2 = f / (h1 + h1);
                for (int j = 0; j < i; j++) {
                	System.out.println("symm8 "+j+" "+i);
                    accu[j] -= h2 * eigenValues[j];
                }
                for (int j = 0; j < i; j++) {
                	System.out.println("symm9 "+j+" "+i);
                    f = eigenValues[j];
                    g = accu[j];
                    for (int k = j; k <= i - 1; k++) {
                    	System.out.println("symm10 "+k+" "+i);
        ...
        for (int i = 0; i < m - 1; i++) {
            ...
            for (int j = 0; j <= i; j++) {
            	System.out.println("symm14 "+j+" "+i);
                eigenVectors[j][i + 1] = 0.0;
            }
        }
        for (int i = 0; i < m; i++) {
        	System.out.println("symm14 "+i+" "+m);
            eigenValues[i] = eigenVectors[m - 1][i];
        ...
        accu[n - 1] = 0.0;
        for (int i = 0; i < n; i++) {
        	System.out.println("symm15 "+i+" "+n);
            piv = Math.max(piv, Math.abs(eigenValues[i]) + Math.abs(accu[i]));
            int l = i;
            while (l < n) {
            	System.out.println("symm16 while "+l+" "+n);
            ...
            if (l > i) {
                int iter = 0;
                do {
                	System.out.println("symm17 while "+Math.abs(accu[i])+" "+(eps*piv));
                    iter = iter + 1;
                    ...
                    for (int j = l - 1; j >= i; j--) {
                        ...
                        for (int k = 0; k < n; k++) {
                        	System.out.println("symm19 while "+k+" "+n);
