...
public class SarsaLearnerMod implements PolicyLearner {
     ...
     */
    public SarsaLearnerMod(double lambda, double gamma, double alpha,
    ...
    public double train() {
        ...
        double delta = reward + gamma * values[nextState][nextAction] - values[state][action];
    ...
    public Policy getPolicy() {
        ...
        for (int i = 0; i < stateCount; i++) {
            ...
            int maxAction = 0;
            boolean ambiguousNextAction = false;

            for (int a = 0; a < actionCount; a++) {
                double actionVal = values[i][a];
                double difference = Math.abs(actionVal - maxActionVal);
                
                if (difference < 0.0001) {
                	ambiguousNextAction = true;
                } else if (actionVal > maxActionVal) {
                    maxActionVal = actionVal;
                    maxAction = a;
                    ambiguousNextAction = false;
                }
            }
            
            if (ambiguousNextAction) {
            	policy[i] = -1;
            } else {
				policy[i] = maxAction;
            }
